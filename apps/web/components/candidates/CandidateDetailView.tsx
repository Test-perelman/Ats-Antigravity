...
import { Mail, Phone, Briefcase, Globe } from 'lucide-react';
...
export default function CandidateDetailView({ candidateId }: CandidateDetailViewProps) {
    const { userData } = useAuth();
    const [candidate, setCandidate] = useState<any>(null);
    const [loading, setLoading] = useState(true);

    useEffect(() => {
        if (!userData?.teamId || !candidateId) return;

        const fetchCandidate = async () => {
            try {
                const docRef = doc(db, 'teams', userData.teamId, 'candidates', candidateId);
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="md:col-span-1">
                <NotesList parentId={candidateId} parentType="candidates" />
            </div>
        </div>
    );
}
